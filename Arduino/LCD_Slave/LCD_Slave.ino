...
void cursorBlink()
{
  switch (encoderCurrentMenu)
  {
    ...
    case MENU_CALIBRATION:
      lcd.setCursor(0,0);
      //lcd.setCursor(10 * ((encoderSelector % 5) % 2), (encoderSelector % 5) / 2);
      break;
    case MENU_SET_VALUE:
      lcd.setCursor(19, 0);
      break;
...
void doEncoderA() {
  ...
  if ( digitalRead(ENCODER_CLOCKWISE) != A_set) { // debounce once more
    ...
    {
      encoderSelector--;
      if (encoderCurrentMenu == MENU_CONFIG) refreshMenu = true; // Sensor menu has to be refreshed to allow scrolling
...
void setup() {
  ...
  addConfigParam(2, "Max weight", 10, PARAM_WEIGHT_MAX);
  addConfigParam(3, "Min weight", 10, PARAM_WEIGHT_MIN);
  addConfigParam(4, "Sedim time", 10, PARAM_SEDIMENTATION_TIME);
  addConfigParam(5, "Filled time", 11, PARAM_FILLED_TIME);
