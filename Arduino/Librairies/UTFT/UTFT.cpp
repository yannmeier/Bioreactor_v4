/*
  ...
  Based on the demo-code provided by ITead studio (for the ITDB02 modules) and 
...
/*------------------------------------------------------------------------
Library already modified for AT90USB1286 (Teensy 2++)
--------------------------------------------------------------------------
*/
...
UTFT::UTFT(byte model, int RS, int WR,int CS, int RST, int SER){ 
	switch (model){
		...
		case SSD1289:													//
			disp_x_size=239;
			disp_y_size=319;
			display_transfer_mode=16;						// 16-bit Mode
			break;
		case ILI9325C:
			disp_x_size=239;
			disp_y_size=319;
		...
		case SSD1289LATCHED:									//
			disp_x_size=239;
			disp_y_size=319;
			display_transfer_mode=LATCHED_16;		// 16-bit Mode
			break;
		case ILI9320_8:
			disp_x_size=239;
			disp_y_size=319;
		...
		case SSD1289_8:												//
			disp_x_size=239;
			disp_y_size=319;
			display_transfer_mode=8;						// 8-bit Mode
			break;
	}
	display_model=model;
...
void UTFT::LCD_Write_COM(char VL){   
	if (display_transfer_mode!=1) cbi(P_RS, B_RS);
